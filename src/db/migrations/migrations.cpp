...
std::vector<bool (*)(const std::shared_ptr<Logger::Logger>&, const std::shared_ptr<Database>&, DBType)> bossMigrations = {
...
bool runVoidCommandsSync(const std::shared_ptr<Logger::Logger>& logger, const std::shared_ptr<Database>& db,
    ...
    for (auto & sqlCmd : commands) {
        ...
        Result result = async::runManualTaskSync(db->runCommand(std::move(command)));

        if (result.getStatus() != DBResultStatus::SUCCESS) {
            // Rollback transaction
            command = Database::craftVoidCommand(rollbackCommand);
            async::runManualTaskSync(db->runCommand(std::move(command)));
