...
bool migration_initial(const std::shared_ptr<Logger::Logger>& logger, const std::shared_ptr<Database>& db, DBType type) {
    ...
    switch (type) {
        case DBType::SQLITE3:
            ...
            sqlCmds.emplace_back("CREATE TABLE friendships (pid INTEGER NOT NULL, friend_pid INTEGER NOT NULL, becameFriends TEXT NOT NULL,"
            ...
            sqlCmds.emplace_back("CREATE TABLE user_info (pid INTEGER NOT NULL, show_presence INTEGER NOT NULL DEFAULT (1), "
                                 ...
                                 "nnaInfo BLOB NOT NULL, presence BLOB NOT NULL, comment BLOB NOT NULL, PRIMARY KEY (pid), "
                                 "lastOnline TEXT NOT NULL, FOREIGN KEY(pid) REFERENCES users(pid));");
