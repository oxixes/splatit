...
void parseChunked(const std::vector<uint8_t>& data, std::unordered_map<std::string, std::vector<std::string>>& headers,
    ...
    while (pos < headerLength + length) {
        auto end = data.begin() + (std::ptrdiff_t) headerLength + (std::ptrdiff_t) length;

        std::string delimiterStr = "\r\n";
        auto chunkHeaderEnd = std::search(data.begin() + (std::ptrdiff_t) pos, end,
        ...
        auto chunkLength = std::search(data.begin() + (std::ptrdiff_t) pos, chunkHeaderEnd,
        ...
        std::string chunkSizeStr(reinterpret_cast<const char*>(data.data() + pos),
                                 chunkLength - (data.begin() + (std::ptrdiff_t) pos));
        ...
        if (chunkSize == 0) {
            ...
            auto chunkEnd = std::search(chunkHeaderEnd + 2 + (std::ptrdiff_t) chunkSize, end,
                                        delimiterStr.begin(), delimiterStr.end());

            if (chunkEnd == end) throw MalformedException("Chunk end not found");
            if (chunkEnd != chunkHeaderEnd + 2 + (std::ptrdiff_t) chunkSize) throw MalformedException("Chunk size mismatch");

            body.insert(body.end(), chunkHeaderEnd + 2, chunkEnd);
        }

        pos += chunkHeaderEnd - (data.begin() + (std::ptrdiff_t) pos) + 2 + chunkSize + 2;
    }
}
